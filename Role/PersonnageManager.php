...
class PersonnageManager
{
  ...
  {
    $q = $this->db->prepare('INSERT INTO personnage(nom, type) VALUES(:nom, :type)');
  ...
  public function get($info)
  {
    if (is_int($info))
    {
      $q = $this->db->query('SELECT id, nom, degats, timeEndormi, type, atout FROM personnage WHERE id = '.$info);
      $perso = $q->fetch(PDO::FETCH_ASSOC);
    }
    else
    {
      $q = $this->db->prepare('SELECT id, nom, degats, timeEndormi, type, atout FROM personnage WHERE nom = :nom');
      $q->execute([':nom' => $info]);
  ...
  public function getList($nom)
  {
    ...
    $q = $this->db->prepare('SELECT id, nom, degats, timeEndormi, type, atout FROM personnage WHERE nom <> :nom ORDER BY nom');
    $q->execute([':nom' => $nom]);
  ...
  {
    $q = $this->db->prepare('UPDATE personnage SET degats = :degats, timeEndormi = :timeEndormi, atout = :atout WHERE id = :id');
